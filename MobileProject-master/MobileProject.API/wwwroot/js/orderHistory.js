$(document).ready(function () {
    ...
    $("#searchUserName").val(user.userName);
});
let user;
function loginUser() {
...
// ✅ Load Orders
function loadOrders() {
    axios.get(`${apiBaseUrl}/GetOrderViewByCondition?userId=${user.id}`)
...
function setupGrid(orders) {
    const columnDefs = [
        //{ headerCheckboxSelection: true, checkboxSelection: true, width: 50, visibled: false },
...
function searchOrders() {
    ...
    var query = `?userName=${filters.userName}&userId=${user.id}&transactionCode=${filters.transactionCode}&status=${filters.status}`;
    if (filters.dateFrom)
...
function resetFilters() {
    $("#searchTransactionCode,  #searchStatus, #searchDateFrom, #searchDateTo").val("");
    searchOrders();
}
