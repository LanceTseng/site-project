...
public class OrdersRepository : IOrdersRepository
{
    ...
    public async Task<IEnumerable<OrderView>> GetAllOrdersViewAsync()
    {
        using var connection = new SqlConnection(_connectionString);
        const string query = "SELECT * FROM OrderView";
        return await connection.QueryAsync<OrderView>(query);
    }

    public async Task<IEnumerable<OrderView>> GetOrderViewByConditionAsync(
        string? userName,
        string? transactionCode,
        string? status,
        int? userId,
        ...
        DateTime? dateTo = null)
    {
        ...
        WHERE (@UserName IS NULL OR o.UserName LIKE '%' + @UserName + '%')
        AND (@TransactionCode IS NULL OR o.TransactionCode LIKE '%' + @TransactionCode + '%')
        AND (@Status IS NULL OR o.Status = @Status)
        AND (@UserId = 0 OR o.UserId = @UserId)");
        ...
        return await connection.QueryAsync<OrderView>(query.ToString(), parameters);
    }
}