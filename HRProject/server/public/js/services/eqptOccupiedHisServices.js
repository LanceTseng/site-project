...
const baseURL = "api/eqpt-occupied-his/";
...
export const getTaskByUserId = async (taskId) => {
  try {
    const response = await axiosInstance.get(`${baseURL}userid/${taskId}`);
    return response.data;
  } catch (error) {
...
export const getTaskByEqptId = async (taskId) => {
    try {
      const response = await axiosInstance.get(`${baseURL}eqptid/${taskId}`);
      return response.data;
    } catch (error) {
      console.error(`Error fetching task with ID ${taskId}:`, error);
      throw error;
