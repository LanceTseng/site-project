...
class TicketViewRepository {
  ...
  async getTicketsByCondition(ticket_topic, department_id, status, user_id, head_id) {
    try {
      const rows = await db.query(
             ...
             ANS (:ticket_id IS NULL OR ticket_id = :ticket_id)`,
        {
          replacements: {
            ...
            ticket_id: head_id ? head_id : null
          },
          type: QueryTypes.SELECT,
        }
      );
  ...
  async getAllTicketHeadView() {
    try {
      return await db.query(`SELECT distinct ticket_id,
                ticket_topic,
                ticket_description,
                ...
                ticket_status_name,
                ticket_request_date,
                ticket_request_by_id,
                ticket_request_by_name,
  ...
  async getTicketHeadViewByCondition(
    ticket_topic,
    department_id,
    status,
    user_id
